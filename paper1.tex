...
Sophize is a novel mathematics library and discussion platform. The platform has been developed by the author over the course of last two years at \url{https://sophize.org}. Sophize's primary mission is to help users find existing proofs of mathematical statements, to discover new proofs, and to utilize this knowledge in their work. We combine knowledge from multiple resources and have currently accumulated, thousands of definitions, theorems and proofs from a wide variety of sources including some published research, PlanetMath, Wikipedia, and the Metamath formal system.
...
Sophize organizes mathematical knowledge as a dense graph of propositions and logical arguments that is built by combining knowledge from a number of documents and other data sources. We use this graph and combine it with the set of foundations and verifications chosen by each user to create proofs tailored to their needs. This introductory video gives an overview of how a mathemtician would browse such proofs can help: \url{https://youtu.be/Wb1JbW9Otek}.

\begin{figure}[ht]
\begin{center}
\fbox{\includegraphics[height=14cm]{proof_tree}}
\caption{Proof shown as a graph of arguments and propositions on the Sophize platform.}
\label{proof_tree}
\end{center}
\end{figure}
...
Section 3 presents Sophize markdown as a lightweight interface for represeting math content and Section 4 discusses its collaboration facilities. Section 5 concludes the paper.


\subsection*{Acknowledgements}
This publication resulted (in part) from research supported by the International Mathematical Union. We would like to thank Patrick Ion and Michael Kohlhase for helping us in organizing our ideas and structuring the content of the paper.

\section{Preliminaries}

\subsection{Sophize's Datamodel}
...
A \textbf{resource} is an abstract concept inherited by all other top-level concepts like terms, propositions, and arguments. Each resource has a URI and contains fields such as search tags and citations.
...
The following example shows the above options in use. Note that we assume that the dataset-id of URIs is available from the context and are thus not specified in the URIs.
...
\paragraph{\textbf{NO\_LINK}: NO\_LINK creates a non-clickable element that whose hover text is the URI of the resource.}
...
We have presented Sophize Markdown, a lightweight language for representing mathematical knowledge on the web. Using this language, we can easily embed mathematical entities like definitions, theorems and proofs, making it very convenient to search and browse the content. While there are other ways to represent semantic knowledge such as content MathML and OpenMath, their scope is somewhat limited to specifying meaning of mathematical formula. The sTeX system \cite{Kohlhase2008} is perhaps most similar to Sophize Markdown but it needs creation of \LaTeX\space documents and use of \LaTeX ML systems. This makes it quite challenging to use in real-time web workflows that the Sophize platform.

We utilized the Sophize Markdown language to create an innovative mathematics communication interface. It specially designed to aid online mathematics collaborations and can help actualize their vast latent potential. We believe that it can only aid large-scale collaborations like Polymath but will also be useful for private collaborations and workshops like those organized by AiM.

In the future, we want to make a substantial portion of math literature embedded with structured data available at a much greater scale. Thus, we will improve Sophize Markdown's support for \LaTeX\space features like equation numbering, tables, and references. In addition, we are developing an online tool to easily extract content like definitions, theorems, proofs from math literature by automating some of the tedious tasks. This process will be aided by the supervised learning techniques \cite{ginev2019scientific} for classification scientific statements.
